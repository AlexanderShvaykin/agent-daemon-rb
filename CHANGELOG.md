...
- Each drained batch commits in one transaction behind a committed-seq watermark, so re-applying a batch writes nothing twice. A batch that fails is rolled back, logged once as a `[History]` error with its record count, and dropped; retry and gap logging come in Story 5.3. A writer that cannot start degrades history like an open failure does.
...
- `history.shutdown_flush_seconds` is now consumed: shutdown waits that long for the writer's final drain, and leaves the store open with a warning if it did not finish.
