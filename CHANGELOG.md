...
- History is an observer: any failure to open it is one `[History]` log line and `history_state == :degraded`, and the fleet and console run on. `enabled: false` creates nothing on disk.
- The supervisor now writes history (Story 5.2). One `History::Writer` thread, named `history_writer`, subscribes to the `EventBus` before any supervised entity spawns and persists each run (`picked_up` → `started` → `finished`, correlated per entity; only observed events are stored), its ordered lifecycle events, and every restart action (coalesced actor set, request and completion time, source and target generation). Restart actions are no longer in-memory only. Timestamps are stored as ISO-8601 UTC with milliseconds.
- Each drained batch commits in one transaction behind a committed-seq watermark, so re-applying a batch writes nothing twice. A batch that fails is rolled back, logged once as a `[History]` error with its record count, and retried (see the Story 5.3 entry below). A writer that cannot start degrades history like an open failure does.
- The history connection waits on a held lock with sqlite3's `busy_handler_timeout`, which releases the GVL between retries, instead of `busy_timeout`, which held it and would have frozen every runner thread for the wait. `History::Database#busy_timeout_ms` exposes the configured value, since `PRAGMA busy_timeout` reads 0 once a handler is set.
- `history.shutdown_flush_seconds` is now consumed: shutdown waits that long for the writer's final drain, and leaves the store open with a warning naming the number of unflushed accepted records if it did not finish (Story 5.3).
- History no longer loses data silently (Story 5.3). A batch that fails to commit is re-applied up to `history.write_retry_count` times, sleeping `min(100 ms × 2^(n-1), write_retry_backoff_ceiling_ms)` before retry `n`; when retries run out, one `[History] gap:` error names the record count, seq range and retry count, and the writer carries on with later batches. Bus records evicted before the writer read them are one `[History] gap:` warn with the count; nothing is synthesized for them. Log lines never carry field values.
- A writer that lost a batch, or whose thread died unasked, is degraded for the rest of its life, and `Master#history_state` reports `:degraded`.
- Schema v2 adds `run.incomplete`. Before its first write the writer marks every run a previous master left open as `incomplete = 1` (logged once with the count), and a run displaced by a newer run of the same entity is marked the same way; `reason` and `finished_at` stay NULL. A restarted master's rows coexist with the old history. The migration is one-way: an older gem refuses a v2 store (`UnsupportedSchemaError`) and runs with history degraded.
- History stores agent output and error detail (Story 5.4). Schema v3 adds `run_output` (`run_id`, `seq`, `stream`, `text`, unique per run and seq) and the `run` columns `output_truncated`, `output_incomplete` and `error_summary`. The writer observes the same redacted `OutputPipeline` the live tail reads through a bounded, non-blocking `History::OutputQueue`, so producers never wait on SQLite. A run's lines commit before, or with, its completion. Each run keeps at most `output_buffer_bytes` of text: the oldest rows go first, never the newest, and `output_truncated` is set. Lines evicted from the queue set `output_incomplete`. A `failed` run gets `error_summary` as JSON `{"reason":"failed","attempt":N,"last_stderr":...}`; other reasons leave it NULL. `last_stderr` is the last *retained* stderr line, so once `output_truncated` or `output_incomplete` is set it can be older than the real last line, or null. The per-run cap bounds each run, not the store: total growth is bounded only by retention (Story 5.6). Output no run can claim, or still pending at a stop, is dropped with one `[History] gap:` warn carrying counts only. The migration is one-way: an older gem refuses a v3 store and runs with history degraded.
- The console browses persisted history (Story 5.5). `GET /history` lists every persisted run newest first (workflow, entity, work item, attempt, generation, start time and a text result label: `ok`, `failed`, `timeout`, `killed`, `incomplete`, or `running` when no finished event is recorded and the run is not marked incomplete — while the writer is healthy that means the run is still open; a writer that degraded mid-run leaves such rows `running` until the next master marks them `incomplete`). `GET /history/entity?id=` shows one entity's runs and its restart actions, and `GET /history/run?id=` shows one run's identity, transitions, error summary and captured output, with explicit notices for a truncated beginning, incomplete capture, and no output. Paging is exclusive seek pagination on `(started_at, id)` with `history.page_size` rows per page (now consumed), so runs inserted between pages never repeat. Entities no longer in the fleet stay listed with an "absent from current fleet" badge. Every page gains a primary Fleet / History navigation, and the entity page links to its persisted history. History pages are static snapshots without the live-update script; since they carry no stream, a new `HistoryAuthorization` middleware between `Auth` and `App` re-checks GitLab group membership on every history request (still at most one GitLab lookup per 60 s recheck interval), so a revoked operator is redirected to login and loses access to persisted output within that interval instead of at `session_ttl`.
- The console reads through `History::Reader`, a second, lazily opened, `query_only` SQLite connection built by the master next to the writer and closed before it at shutdown; the writer's connection is never shared. A malformed or unknown id is the existing non-disclosing 404. When history is disabled or a read fails, the history pages answer 503 "History unavailable" and log `[Console] history read failed: <ExceptionClass>` only; the next request reopens the connection, and the writer and the fleet are unaffected.
- History expires automatically (Story 5.6). The history writer thread prunes at startup and then every `history.prune_interval_seconds`, deleting runs (with their events and output) that finished more than `history.retention_days` ago, crash-incomplete runs by start time (the live master's open runs are never pruned), restart actions by request time, and entities no row references that are no longer in the config — at most `history.prune_batch_size` per transaction, with ordinary writes serviced between batches. The three retention keys are now consumed. No cron, no second process, no `VACUUM`: the file does not shrink, and freed pages are reused. A failed batch rolls back, logs `[History] prune failed: <Class>: <message>`, and is retried at the next interval; shutdown starts no new prune.
- `/history` shows the retention period and the last successful prune ("Not yet run" until the first cycle after a master start), and every history page shows a "History degraded:" warning (`role="status"`, text, not colour) when the writer lost data, the last prune failed, or its status could not be read. The fleet and entity pages never show it, and `/healthz` is unchanged. `History::Writer#status` and `History::Reader#status` expose the snapshot; `Master::HISTORY_READER_FACTORY` now takes `(history_config, status)`.
- `docs/deployment.md` puts the database and its `-wal`/`-shm` files in the threat model and describes retention as a security control.
